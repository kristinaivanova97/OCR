# This code was taken from https://github.com/stackchain/ocr-cnh-2-json with small changes
import imutils
...
from utils import rect2Box, validDateString
import pytesseract
...
def cropRois(image, rects, multHeight=0.73, multWidth=0.97, topHeightCrop=30):
    ...
    for r in rects:
        ...
        if (ratio > 2 and ratio < 16):
            ...
            int(croppedW * multWidth), int(croppedH * multHeight if croppedH < topHeightCrop else croppedH * 0.9)),
                                               (size[0] / 2, size[1] / 2))
